...
>
> 无需编程、无需记忆命令——在对话中用**自然语言**描述你的 Meta 分析需求，技能自动完成全套分析。底层基于 R 与 14 核心 + 2 可选专业 R 包（metafor、meta、netmeta、bayesmeta、dosresmeta、mada 等），输出语言随操作系统语言自动切换（中文系统出中文，否则出英文；可随时强制切换）。默认采用**安全预览**模式——只展示生成的 R 代码、不执行；确认无误后说出触发词即可真正计算。

---

> **出站数据披露（ct-base §5，强制）** — 本技能会把您的**分析数据**（研究事件数 / 样本量 / 效应量等，不含任何个人身份信息）发送到 coze 元分析端点（默认 `https://ct-meta.coze.site/run`）执行云端 R 计算。默认端点已预置在 `adapters/config.json` 的 `auto_approve_endpoints` 白名单；自定义端点（`COZE_META_ENDPOINT`）首次使用会弹确认提示（AUTH-BLOCK），您确认后写入白名单、后续免确认。若不同意发送，技能自动回退本地引擎（无本地 R 时返回"本次未使用云端分析"的明确提示）。发送前 payload 会经脱敏处理（剥离个人身份信息）。
>
> **随请求发送的元数据（ct-base §5）**：请求还携带 `query_origin`（本机主机名的 SHA-256 哈希，仅用于服务端归因 / 限流，非明文主机名）与 `locale`（OS 语言，用于双语输出）。两者均不用于识别您的个人身份。
>
> **错误报告端点披露（ct-base §5 / §20.3，强制）** — 当您确认通过技能内置的错误报告器（`adapters/bug_report.py`）发送一份（脱敏）错误报告时，技能**仅**把 11 键白名单信封（技能名 / 版本 / 错误类型 / 错误码 / 引擎状态 / 您填写的自由文本 `description` / locale / `query_origin` / 会话指纹 / 重试次数 / 检验名）发送到统一的错误报告端点 `https://ct-bugreport.coze.site/run`。它**不含任何分析数据与个人身份信息**——`description` 是唯一自由文本字段，且您在确认前会审阅（硬边界：不含可识别个人 / 机构 / 受试者信息）。上述 `query_origin` + `locale` 元数据同样随附。若您拒绝，则什么都不发送；若本次会话无云端调用，报告改为本地保存（`save_local_report`，数据不出本机）。
...
以下 7 个真实对话示例，从简单到高级排列。每个示例含 **「你说」** 和 **「助手回复（示意）」**，以及如何触发真实计算。
...
**📌 获取真实结果：** 说 **「请直接计算」** —— 助手将请求发送至云端 coze R 引擎并返回真实数字（详见第 4 节「安全预览」）。
...
### 示例 7 · 选题：选择候选 Meta 分析方向（Topic Selection 上游门控）
**你说：**
...
A: 在对话中说 **"请直接计算"** 或 **"执行"** —— 助手会将分析请求发送至云端 coze R 引擎并给出数字。这是默认的安全设计：先看请求，确认无误再计算。**发送只发生在你明确说出这句高摩擦指令时**——技能不会自行发送请求，也不会因闲聊中偶然提到这些词就触发。
...
**Q: 如果数据要保密怎么办？**
A: 本技能只把您的**分析参数 / 汇总统计量**（事件数、样本量、效应量等）发送至云端 coze R 引擎，从不接触您的原始数据集或个体级数据。若您仍希望数据不出本机，可走**本地引擎**（说「用本地引擎」或设置 `prefer="local"`）：相同的 R 引擎在本机运行，数据不出机器。您也可以要求输出完整可复现 R 代码，自行用真实数据在本地运行。

## 4. 安全预览（Safe Preview）

- **默认行为：** 技能只**生成并展示分析请求（task / data / params / figure 信封），不发送**——你可以先审查方案，确认无误后再让它运行。
- **触发真实计算：** 在对话中说 **"请直接计算"** 或 **"执行"** → 助手将请求信封发送至云端 coze R 引擎，由其计算并返回真实数字。（没有 `--yes` 参数——发送完全由这句自然语言触发；技能不会自行发送，也不会因闲聊中偶然提到这些词就触发。）
- **只看代码：** 说 **"展示代码"** 或 **"仅预览"** → 只给请求信封，不给结果。
- **默认计算路径**：本技能默认将分析请求发送到云端 coze R 引擎（`https://ct-meta.coze.site/run`）执行，分析数据按上方「出站数据披露（§5）」块发送；如需本地 / 离线分析，走本地引擎（`prefer="local"`）。安全预览仅控制「是否发送请求」，与计算位置无关。
...
> 测试日期 2026-08-20，依据 ct-base §16.6 逐示例实测：**7/7 通过**。计算类示例 1/2/3/6 经 coze 端点（`https://ct-meta.coze.site/run`）真实返回 `stats` + `figures` + `repro`；行为类示例 4/5/7 与 SKILL.md Triage（§5.2）及 `references/interactive_menu.md` / `references/topic-selection.md` 一致。完整报告见 `meta_readme_test/README_EXAMPLES_TEST_REPORT.md`。

| 示例 | 类型 | 实测状态 |
|---|---|---|
| 1 二分类 OR 配对 / 2 效应量转换 / 3 SMD+亚组 / 6 PRISMA 流程图 | 计算类（coze） | ✅ 通过 |
| 4 网络 Meta 路由菜单 / 5 Vague grill-me / 7 选择候选方向 | 行为类（路由/选题） | ✅ 通过 |
...
**版本**: v1.12.2 | **许可**: MIT | **作者**: medstatstar, phoe-zip
...
> CT 全系列技能由 20+ 个技能构成，按「保密信息出域风险 + 是否对外检索」分为 **A、B 两档（原 C/D 档已并入 B 档·涉密）**，完整覆盖新药临床试验（Clinical Trial）全流程的各方面需求。
>
...
> - **B 档（涉密·内部，原 C/D 档）**：涉及药企需严格保密的临床试验数据、内部资讯等敏感内容（如 ct-analysis、ct-sdtm、ct-eligibility 等）；B 档在本地处理（`egress=none`，数据不出域）或需审批出站（`egress=approval-req`，如 ct-eligibility）。B 档技能仅限企业内部使用，目前不对外公开发布。
>
